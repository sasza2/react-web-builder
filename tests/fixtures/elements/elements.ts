...
export const getElements = async (page: Page) => {
	const elements = await page
		.locator(".react-panzoom-element:not(.react-panzoom-element-is-shadow)")
		.all();
	return elements;
};

const getElementIdByLocator = async (
	element: Locator,
): Promise<string | null> => {
	const classNames = await element.getAttribute("class");
	const classList = classNames.split(" ");
	const idClassName = classList.find((className) =>
		className.startsWith("react-panzoom-element--id-"),
	);
...
export const getElementDetails = async (
	...
	const roundHeight = (value: number) =>
		Math.ceil(Math.floor(value * 10000) / 10000);

	const width = Math.round(
		(elementPosition.width / gridInPosition.width) * breakpointDetails.cols,
	);
	const height = roundHeight(
		elementPosition.height / (breakpointDetails.rowHeight * zoom),
	);
	const top = Math.round(
		(elementPosition.y - gridInPosition.y) /
			(breakpointDetails.rowHeight * zoom),
	);
	const left = Math.round(
		((elementPosition.x - gridInPosition.x) / gridInPosition.width) *
			breakpointDetails.cols,
	);
