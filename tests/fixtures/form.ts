...
const editFieldRichText = async (formProperty: Locator, value: EditFieldRichTextValue[]) => {
  const textbox = formProperty.frameLocator('[data-testid="richTextFrame"]')
    .getByRole('textbox');

  for await (const action of value) { // eslint-disable-line no-restricted-syntax
...
const editEdges = async (formProperty: Locator, value: EdgesValue, edgeFieldName: string) => {
  const fields = ['left', 'right', 'top', 'bottom'] as const;

  for await (const field of fields) { // eslint-disable-line no-restricted-syntax
...
export const editPadding = (formProperty: Locator, value: EdgesValue) => editEdges(formProperty, value, 'padding');

export const editColor = async (formProperty: Locator, value: string) => {
  const colorPicker = formProperty.getByTestId('colorPicker');
  await colorPicker.click();

  const option = colorPicker.locator(`[data-testid*="color--${value.toLowerCase()}"]`);
...
export const editBorder = async (formProperty: Locator, value: EditBorderValue) => {
...
export const editField = async (container: Locator, label: string, value: unknown) => {
  const labelElement = container.getByText(label, { exact: true });
  expect(labelElement).toHaveCount(1);

  const formProperty = container.locator('[data-testid*="formProperty--"]', { hasText: label }).first();
  expect(formProperty).toHaveCount(1);

  const testId = await formProperty.getAttribute('data-testid');
  expect(testId).toBeTruthy();

  const formPropertyTestId = testId.split(' ').find((part) => part.startsWith('formProperty--'));
