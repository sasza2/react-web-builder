...
test('publish template', async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot('templatePublishedDesktop.png', { fullPage: true });

  await page.setViewportSize({ width: 360, height: 600 });
  await expect(page).toHaveScreenshot('templatePublishedMobile.png', { fullPage: true });
});
