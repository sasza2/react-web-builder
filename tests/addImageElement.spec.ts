...
const IMG_SRC =
	"https://raw.githubusercontent.com/sasza2/react-grid-panzoom/master/docs/preview.gif";
...
test("add image element", async ({ page }) => {
	...
	await editField(properties, "Image source", IMG_SRC);

	await sleep(500);

	expect(await toSnapshot(elementIn)).toMatchSnapshot("addImageElement.txt");

	await editField(properties, "Link destination", LINK_DESTINATION);

	await sleep(500);

	expect(await toSnapshot(elementIn)).toMatchSnapshot(
		"addImageElementWithLink.txt",
	);
