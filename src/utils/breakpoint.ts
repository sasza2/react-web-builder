...
export const getBreakpointBackgroundColor = (
	breakpoint: Breakpoint,
	pageSettings: PageSettings,
) => breakpoint.backgroundColor || pageSettings.backgroundColor;
...
export const shouldLoadTemplateForBreakpoint = (
	...
	const elementsExtrasIds = Object.keys(
		get(page, `elementsExtras.${breakpoint.id}`, {}),
	);
...
export const shouldLoadTemplate = (page: Page) => {
	...
	return page.breakpoints.some((breakpoint) =>
...
export const initElementsExtrasFromBreakpoint = (
	...
	if (breakpoint && !elementsExtras.current[breakpoint.id]) {
		elementsExtras.current[breakpoint.id] =
			page?.elementsExtras?.[breakpoint.id] || {};
	}
};
...
export const createTreeFromBreakpoint = ({
	...
	selectedElements.forEach((element) => {
		...
		if (element.componentName === "Container") {
			...
			const copyContainerIdPropValue = rewriteContainersIds
				? createUniqueId()
				: container.id;
...
export const byBreakpointId = (id: BreakpointId) => (breakpoint: Breakpoint) =>
	breakpoint.id === id;
