...
export const getColorForSketch = (color?: string, defaultValue?: string) => {
	...
	if (colorType === ColorType.Gradient) {
		...
		if (gradientColor.colors.length > 0)
			return normalizeColor(gradientColor.colors[0].color);
	}
...
export const getColorForInput = (color?: string, defaultValue?: string) => {
	const colorForInput = getColorWithoutHash(
		getColorForSketch(color, defaultValue),
	);
	if (colorForInput.length === 8 && colorForInput.endsWith("ff"))
		return colorForInput.substring(0, 6);
...
export const normalizeSketchColor = (colorResult: ColorResult) => {
	...
	const getHexAlpha = (opacityPercentage: number) =>
		`0${Math.round((255 / 100) * opacityPercentage).toString(16)}`
			.slice(-2)
			.toLowerCase();
