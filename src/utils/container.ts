...
} from "@/components/View/elementsRefMap";
...
export const getContainerExtras = (
	...
	containerMapRef.forEach((elementRef, elementId) => {
		const paddingBottom =
			getElementPaddingFromStyle(DEFAULT_CONTAINER_ID, elementId) || 0;
		const heightInPixels = (
			elementRef.firstChild as HTMLElement
		).getBoundingClientRect().height;

		containerExtras[elementId] = {
			height: Math.ceil(
				Math.round(
					(heightInPixels / container.rowHeight / zoom) * PIXEL_PRECISION,
				) / PIXEL_PRECISION,
			),
			paddingBottom,
		};
...
export const createTreeForContainer = (
	cols: number,
	t: TFunction<"common">,
): Tree => {
	const tree: Tree = {
...
export const createElementsForContainer = (
	...
	const clonedTree = cloneTree(
		createTreeForContainer(container.cols, t),
		(originalId) => {
	...
	const getPaddingBottom = (elementId: string): number => {
		...
		const paddingBottom =
			getElementPaddingFromStyle(DEFAULT_CONTAINER_ID, originalId) || 0;

		return paddingBottom;
	};

	const elements = calculatePositionsOfElements(
		clonedTree,
