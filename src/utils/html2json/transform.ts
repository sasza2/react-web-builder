	...
	const parser = new htmlparser2.Parser({
		onopentag(tagName, attributes) {
			...
			if (attributes.style) {
				...
				if (style) {
					element.style = transformStylesForReact(
						style.nodes as ItemNode[],
						errors,
					);
				}
			}
			...
			if (
				ALLOWED_TAGS[typedTagName] &&
				ALLOWED_TAGS[typedTagName] !== tagName
			) {
		...
		ontext(text) {
			if (openedStyle) {
				...
				if (style) {
					openedStyle.children = style.nodes
						.map((node) => {
							if ("selector" in node) {
