...
export const transformSelector = (
	...
	return selector
		.split(",")
		.map((className) => {
			const classNamesParts = className
				.split(" ")
				.map((item) => {
					if (
						ALLOWED_TAGS[item as keyof typeof ALLOWED_TAGS] &&
						ALLOWED_TAGS[item as keyof typeof ALLOWED_TAGS] !== item
					) {
	...
	try {
		...
		ast.each((selectorNode) => {
			selectorNode.walk((node: Node) => {
				if ((node.type === "class" || node.type === "id") && !node.value) {
					isValid = false;
				}
...
const isValidCSSProperty = (name: string) =>
...
const transformAttributeNameForReact = (name: string) =>
	name
		.split("-")
		.map((item, index) => {
			if (!item) return "";
...
export const transformStylesForReact = (
	nodes: ItemNode[],
	errors?: ErrorsInstance,
): Record<string, string> => {
...
export const parseStyle = (
	...
	try {
		...
		reportError(
			errors,
			wrapError({ type: TransformErrorTypes.InvalidCSS, css }),
		);
		return null;
	}
};
