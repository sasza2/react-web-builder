...
export const borderValidator: Validator = (value) => {
	...
	if (parts.length === 1) {
		return !!(parts[0] === "none" || widthRegex.test(parts[0]));
	}
...
const sizePositiveValidator: Validator = (value) => {
	...
	if (
		!parsedValue ||
		!parsedValue.unit ||
		Number.parseFloat(parsedValue.number) < 0
	)
		return false;
...
export const sizeArrayValidator =
	(maxParts: number): Validator =>
	(value) => {
		const sizeParts = `${value}`
			.split(" ")
			.filter(Boolean)
			.filter(sizeValidator);
...
export const spacingValidator: Validator = (value) => {
	if (allowedValuesValidator("normal", "inherit", "initial", "unset")(value))
		return true;
...
export const flexValidator: Validator = (flex: string) => {
	...
	if (values.length === 3) {
		return (
			numberValidator(values[0]) &&
			numberValidator(values[1]) &&
			isLengthOrAuto(values[2])
		);
	}
...
export const alignItemsValidator = allowedValuesValidator(
	"stretch",
	"flex-start",
	"flex-end",
	"center",
...
export const allowedCSSProperties: Record<string, Validator> = {
	...
	display: allowedValuesValidator(
		"block",
		"inline",
		"inline-block",
		"flex",
	...
	"flex-direction": allowedValuesValidator(
		"row",
		"row-reverse",
		"column",
		"column-reverse",
	...
	"font-style": allowedValuesValidator(
		"normal",
		"italic",
		"oblique",
		"inherit",
	...
	"font-weight": allowedValuesValidator(
		"normal",
		"bold",
		"bolder",
		"lighter",
	...
	"justify-content": allowedValuesValidator(
		"flex-start",
		"flex-end",
		"center",
		"space-between",
	...
	"list-style": allowedValuesValidator(
		"disc",
		"circle",
		"square",
		"decimal",
	...
	"text-align": allowedValuesValidator(
		"left",
		"right",
		"center",
		"justify",
	...
	"text-decoration": allowedValuesValidator(
		"none",
		"underline",
		"overline",
		"line-through",
