...
export function Image({ border, boxShadow, href, url }: ImageComponentProps) {
	...
	const renderImage = () => {
		...
		return <a {...props}>{image}</a>;
	};

	return <div style={{ display: "flex", boxShadow }}>{renderImage()}</div>;
}
...
function VideoIn({ url }: VideoComponentProps) {
	...
	return (
		<VideoWrapper>
			<ReactPlayer src={url.location} width="100%" height="100%" />
		</VideoWrapper>
	);
}
...
export const useInternalComponents = ({
	...
	const components: Array<WebBuilderComponent> = [
		...
		},
		{
			...
			component: Video,
			group: BasicGroup,
			props: [createSourceProperty(defaultVideoSrc || DEFAULT_VIDEO_SRC)],
			order: -300,
		},
	];
	...
	if (defaultButtonAvailable !== false) {
		components.push({
			...
			props: [
				...
				createContentProperty(defaultButtonContent || DEFAULT_BUTTON_CONTENT, {
					colorAvailable: false,
					hyperlinkAvailable: false,
				}),
				createColorProperty("#ffffff"),
				createBackgroundColorProperty(
					defaultButtonBackgroundColor || theme.colors.darkBlue,
				),
				createPaddingProperty({
					top: 15,
					bottom: 15,
