...
export const selectedElementSlice = createSlice({
	...
	initialState,
	reducers: {
		setSelectedElements: (_state, { payload: { elementsIds } }: ActionSet) =>
			elementsIds,
		toggleSelectedElement: (
			state,
			{ payload: { elementId } }: ActionRemove,
		) => {
			if (state.includes(elementId)) {
		...
		replaceSelectedElements: (
			_state,
			{ payload: { elementsIds } }: ActionReplace,
		) => elementsIds,
	},
	extraReducers: (builder) => {
		builder
			.addCase(removeBreakpoint, resetSelectedElements)
			.addCase(setSelectedBreakpoint, resetSelectedElements)
			.addCase(setElementsInBreakpoint, (state, { payload: { elements } }) =>
				removeDeletedElements(state, elements),
			)
			.addCase(
				setElementsInBreakpointProgrammatic,
