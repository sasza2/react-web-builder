...
export const pageSettingsSlice = createSlice({
	...
	reducers: {
		updatePageSettings: (
			state,
			{ payload: { pageSettings } }: ActionUpdate,
		) => ({
			...state,
			...pageSettings,
		}),
		setPageSettings: (_state, { payload: { pageSettings } }: ActionUpdate) =>
			pageSettings,
		replacePageSettings: (_state, { payload }: ActionReplace) => payload,
	},
});

export const { replacePageSettings, setPageSettings, updatePageSettings } =
