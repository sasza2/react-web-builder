...
const updateBreakpoint = (
	state: SidebarState,
	breakpoint: { id: BreakpointId; parentId?: BreakpointId },
) => {
...
export const sidebarSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder
			.addCase(setSelectedElement, (state, action) => {
			...
			.addCase(removeElementsFromBreakpoint, (state) => {
				if (state.view === SidebarView.EditElement)
					state.view = SidebarView.AddElement;
			})
			.addCase(setSelectedBreakpoint, (state, action) =>
				updateBreakpoint(state, action.payload),
			)
			.addCase(addBreakpoint, (state, action) =>
				updateBreakpoint(state, action.payload.breakpoint),
			)
...
export const { replaceSidebar, setSidebarView, setSidebar } =
	sidebarSlice.actions;

type SetViewAnimation = (
	nextView: SidebarView | null,
) => (dispatch: AppDispatch, getState: GetState) => void;

export const setViewAnimation: SetViewAnimation =
	(nextView) => (dispatch, getState) => {
