...
const findLastChangeIndex = (
	state: Changes,
	action: Action<string>,
): number => {
	if (
		action.type !== "elementsInBreakpoints/setElementsInBreakpointProgrammatic"
	)
		return -1;
...
const pushChanges = (
	state: Changes,
	{ payload: { action, key, time } }: ActionPushChange,
) => {
	...
	if (lastElementInBreakpointsIndex >= 0) {
		state.history.splice(lastElementInBreakpointsIndex, 0, {
			action,
			key,
			time,
	...
	reducers: {
		...
		undoChanges: (state) => {
			...
			for (let i = state.index - 1; i >= 0; i--) {
				...
				if (
					prevHistory &&
					prevHistory.action.type ===
						"elementsInBreakpoints/addElementToBreakpoint" &&
					action.type === "elementsInBreakpoints/setElementsInBreakpoint" &&
					time - prevHistory.time < 1000
				) {
					continue;
				}
				...
				if (
					prevHistory &&
					prevHistory.action.type === "pageSettings/updatePageSettings" &&
					action.type === "elementsInBreakpoints/setElementsInBreakpoint"
				) {
					continue;
				}
		...
		redoChanges: (state) => {
			...
			for (let i = state.index; i < state.history.length; i++) {
				...
				if (
					next &&
					next.action.type ===
						"elementsInBreakpoints/setElementsInBreakpoint" &&
