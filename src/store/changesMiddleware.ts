...
const changesMiddleware: Middleware =
	(store: Store) => (next) => (action: Action<ActionName>) => {
		if (action.type === "changes/pushChange") {
			return next(action);
		}

		if (
			action.type === "changes/undoChanges" ||
			action.type === "changes/redoChanges"
		) {
			next(action);
			updateStore(store);
			return;
		...
		if (actions.includes(action.type)) {
			store.dispatch(
				pushChange({ key: createUniqueId(), action, time: Date.now() }),
			);
		}
	};
