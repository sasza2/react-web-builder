...
export const selectedBreakpointSlice = createSlice({
	...
	initialState,
	reducers: {
		setSelectedBreakpoint: (_state, { payload: { id } }: ActionSet) =>
			id || null,
		replaceBreakpoint: (_state, { payload: { id } }: ActionReplace) =>
			id || null,
	},
	extraReducers: (builder) => {
		builder
			...
			.addCase(
				setSelectedElement,
				(state, { payload: { breakpointId } }: ActionSetSelectedElement) => {
			...
			.addCase(
				openContainer,
				(_state, { payload: { breakpointId } }) => breakpointId,
			)
			.addCase(removeAllBreakpoints, () => null);
	},
});

export const { replaceBreakpoint, setSelectedBreakpoint } =
	selectedBreakpointSlice.actions;
