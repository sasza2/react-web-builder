...
export const selectedElementSlice = createSlice({
	...
	initialState,
	reducers: {
		setSelectedElement: (_state, { payload: { elementId } }: ActionSet) =>
			elementId,
		replaceSelectedElement: (
			_state,
			{ payload: { elementId } }: ActionReplace,
		) => elementId,
	},
	extraReducers: (builder) => {
		builder
			.addCase(
				removeElementsFromBreakpoint,
				(state, { payload: { elementsTree } }) => {
					for (const elementTree of elementsTree) {
...
export const { replaceSelectedElement, setSelectedElement } =
