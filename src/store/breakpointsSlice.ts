...
type ActionUpdate = PayloadAction<{
	breakpointId: string;
...
const updateBreakpointAction = (
	state: Breakpoint[],
	{ payload: { breakpointId, breakpoint } }: ActionUpdate,
) => {
...
export const breakpointsSlice = createSlice({
	...
	initialState,
	reducers: {
		addBreakpoint: (state, { payload: { breakpoint } }: ActionAdd) => [
			...state,
			breakpoint,
		],
		addBreakpointSilent: (state, { payload: { breakpoint } }: ActionAdd) => [
			...state,
			breakpoint,
		],
		removeBreakpoint: (state, { payload: { breakpoint } }: ActionRemove) =>
			state.filter(
				(currentBreakpoint) => currentBreakpoint.id !== breakpoint.id,
			),
		updateBreakpoint: updateBreakpointAction,
		updateBreakpointSilent: updateBreakpointAction,
		replaceBreakpoints: (
			state,
			{ payload: { breakpoints } }: ActionReplaceAll,
		) => {
			state.length = 0;
	...
	extraReducers: (builder) => {
		builder
			.addCase(pasteElements, (state, { payload }) => {
			...
			.addCase(removeElementsFromBreakpoint, (state, { payload }) => {
				...
				return state.filter(
					(breakpoint) => !idsContainersToRemove.includes(breakpoint.id),
				);
			});
	},
});
