...
export function Leaf({
	...
	if (hyperlinkAvailable !== false && leaf.link) {
		children = (
			<a style={applyColor(leaf.color)} href={leaf.link}>
				{children}
			</a>
		);
	}
	...
	if (colorAvailable !== false && leaf.color) {
		...
		if (isLightColor(leaf.color)) {
			if (leafClassName)
				leafClassName = `${leafClassName} .${SLATE_HIGHLIGHTED}`;
	...
	return (
		<span {...attributes} className={leafClassName || undefined} style={style}>
			{children}
		</span>
	);
}
