...
export function BreakpointsSelect() {
	...
	const options = useMemo(() => {
		...
		const list = sortedBreakpoints.map((itemBreakpoint, index) => {
			...
			let label = "";
			if (next) {
				if (itemBreakpoint.to === null)
					label = `${itemBreakpoint.from}px - ${next.from}px`;
				else label = `${itemBreakpoint.from}px - ${itemBreakpoint.to}px`;
			} else if (itemBreakpoint.to === null)
				label = `${itemBreakpoint.from}px - 100%`;
			else label = `${itemBreakpoint.from}px - ${itemBreakpoint.to}px`;
		...
		list.push({
			value: "-",
			label: (
				<BreakpointLabel testId="breakpointAdd" label={t("breakpoint.add")} />
			),
		});
	...
	const renderContent = () => {
		if (disabled) {
			return (
				<FieldProvider name="breakpoint" setValue={setValue} value="container">
					<Select
						name="breakpoint"
						size="lg"
