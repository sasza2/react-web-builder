...
type DragElementProps = React.PropsWithChildren<DragElementDetails & {
  onCancel: () => void,
  onSuccess: () => void,
}>;
...
export const DragElement: React.FC<DragElementProps> = ({
  ...
  useEffect(() => {
    ...
    const isOnGridArea = (position: Position) => position.x < maxRight
    ...
    onComponentElementChange.current = (position: Position) => {
      if (isOnGridArea(position)) {
        ...
        onAdd({
          ...
          y: (position.y - originalPanZoomPosition.y - gridTop) / panZoom.getZoom(),
        });
      }
    };
  ...
  return ReactDOM.createPortal(
    <Container
      style={{ height: `calc(100vh - ${gridTop}px)`, top: gridTop }}
    >
      <PanZoom disabledMove disabledZoom ref={panZoomRef} onElementsChange={onElementsChange}>
        <PanZoomElement id={COMPONENT_ID} x={elementPosition.x} y={elementPosition.y}>
          {children}
        </PanZoomElement>
      </PanZoom>
    </Container>,
