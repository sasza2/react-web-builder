...
export function Resizable({
  ...
  const onLoad = useCallback((node: HTMLDivElement) => {
    containerRef.current = node;
    if (node) node.style.setProperty(RESIZABLE_PROP_NAME, `${defaultHeight}px`);
  }, [defaultHeight]);
  ...
  const onStartResizing: React.PointerEventHandler = (startEvent) => {
    ...
    const onMoving = (e: MouseEvent) => {
      ...
      containerRef.current.style.setProperty(RESIZABLE_PROP_NAME, `${nextHeight}px`);
