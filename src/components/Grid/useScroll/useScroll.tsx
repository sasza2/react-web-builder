...
export const useScroll = () => {
  ...
  const onScrollChange = useCallback(() => {
    if (!gridAPIRef.current || !scrollVerticalInRef.current
      || !scrollHorizontalInRef.current || isUsingScrollRef.current) return;
    ...
    const verticalheightPercentage = Math.min(parentRect.height / childRect.height * 100, 100);
    const verticaltopPercentage = Math.abs(childRect.top - parentRect.top) / childRect.height * 100;
    ...
    const horizontalWidthPercentage = Math.min(parentRect.width / childRect.width * 100, 100);
    const horizontalLeftPercentage = Math.abs(childRect.left - parentRect.left) / childRect.width * 100;
  ...
  const onStartScrollVertical: React.PointerEventHandler<HTMLDivElement> = (e) => {
    const panZoom = gridAPIRef.current.getPanZoom();
  ...
  const onStartScrollHorizontal: React.PointerEventHandler<HTMLDivElement> = (e) => {
    const panZoom = gridAPIRef.current.getPanZoom();
    const childRect = panZoom.childNode.getBoundingClientRect();
    const panZoomPosition = panZoom.getPosition();
