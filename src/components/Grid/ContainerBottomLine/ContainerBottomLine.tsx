...
export function ContainerBottomLine({ breakpoint }: ContainerBottomLineProps) {
	...
	const breakpointHeight = useMemo<number | null>(() => {
		...
		const parentBreakpoint = breakpoints.find(
			byBreakpointId(breakpoint.parentId),
		);
		...
		const breakpointHeightProp = element.props.find(
			(prop) => prop.propId === "breakpointHeight",
		);
		if (!breakpointHeightProp) return null;

		const breakpointHeightValue =
			breakpointHeightProp.value as BreakpointHeight;
