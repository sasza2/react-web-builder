...
const breakpointsLastPosition = new Map<
	BreakpointId,
...
export const useGridPositionInit = (isLoaded: boolean) => {
	...
	useEffect(() => {
		...
		if (lastBreakpointPosition) {
			...
			panZoom.setPosition(
				lastBreakpointPosition.position.x,
				lastBreakpointPosition.position.y,
			);
		...
		const breakpointWidth =
			getBreakpointWidth(breakpoint) + padding.left + padding.right;
