...
export function RemoveEmptySpaceBelow({ onClose, row }: MoveElementsBelowHereProps) {
  ...
  const [isVisible] = useState(
    () => {
      if (hasAnyElementAtRowPosition(elements, row, gridAPIRef.current.measureElementHeight)) {
        return false;
      }
  ...
  const onRemoveEmptySpaceBelow = () => {
    ...
    const elementsBelowMap = groupElementsById(getElementsBelowRow(elements, row));
