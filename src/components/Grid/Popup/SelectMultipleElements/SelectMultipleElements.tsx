...
export function SelectMultipleElements({ onClose, row }: MoveElementsBelowHereProps) {
  ...
  const appendElements = (func: typeof getElementsAboveRow, rowOffset: number) => {
    const combined = new Set(
      [...func(elements, row + rowOffset).map((element) => element.id), ...selectedElements],
    );
  ...
  return (
    <>
      ...
      <PopupItem onClick={onDeselectAll}>
        {t('element.deselect')}
      </PopupItem>
      ) }
      { isVisibleAbove && (
