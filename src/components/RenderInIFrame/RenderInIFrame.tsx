...
const ApplyHeightToIFrameBody = forwardRef<
	ApplyHeightToIFrameBodyRef,
...
export function RenderInIFrame({ children }: React.PropsWithChildren) {
	...
	const onChange = useCallback((height: number) => {
		if (updateHeightInIFrameRef.current)
			updateHeightInIFrameRef.current.setHeight(height);
	}, []);

	return (
		<Resizable defaultHeight={160} minHeight={160} onChange={onChange}>
			<Frame
				style={{
					opacity: ready ? 1 : 0,
					height: `var(${RESIZABLE_PROP_NAME})`,
				}}
