...
function EditBreakpointIn() {
	...
	const buttons = (
		<Buttons>
			<RemoveButton onClick={onRemove}>{t("breakpoint.delete")}</RemoveButton>
		</Buttons>
	);

	return (
		<>
			<SidebarHeader onBack={onBack}>{t("breakpoint.edit")}</SidebarHeader>
			<SidebarScrollbar>
				<FormContainerDiv>
					<FormProvider {...formCreator}>
						<Form
							errors={errors}
							onClearBreakpoint={
								elements.length ? onClearBreakpoint : undefined
							}
							onSave={onSave}
						/>
					</FormProvider>
