...
import type { Breakpoint, WebBuilderElements } from "types";
...
import type { Errors, IForm } from "./types";

type UseValidateForm = () => [Errors, (form: IForm) => Errors];

const validateIsNumber = (value: string | number) =>
	!Number.isNaN(Number.parseInt(value as string));

const validateIsGt = (value: string | number, min: number) =>
	(value as unknown as number) >= min;

const validateIsLt = (value: string, max: number) =>
	(value as unknown as number) < max;

const validateHorizontalPadding = (
	paddingLeft: number,
	paddingRight: number,
	width: number,
) => width - paddingLeft - paddingRight >= 320;
...
const validateIfBreakpointExists = (
	...
	return breakpoints.filter(isBreakpoint).some((breakpoint) => {
		if (currentBreakpoint && breakpoint.id === currentBreakpoint.id)
			return false;
		return valueAsNum === breakpoint.from;
	});
};

const validateIfColumnsFitElements = (
	cols: number,
	elements: WebBuilderElements,
) => elements.filter((element) => element.x + element.w > cols).length === 0;
...
export const useValidateForm: UseValidateForm = () => {
	...
	const validate = (form: IForm) => {
		...
		if (nextErrors.length) {
			setTimeout(() => {
				const errorNodes = sidebarRef.current.querySelectorAll(
					'[data-builder-error="true"]',
				);
				...
				sidebarRef.current.scrollTo({
					top:
						errorRect.top -
						(sidebarRect.top - sidebarRef.current.scrollTop) -
						NAVBAR_HEIGHT, // TODO
					behavior: "smooth",
				});
			}, 100); // TODO
