...
const transformError =
	(t: TFunction<"common">) =>
	(error: TransformError): string => {
		switch (error.type) {
			case TransformErrorTypes.InvalidCSS:
				return t("element.html.errors.invalidCSS", {
					css: error.css.substring(0, CSS_MAX_LENGTH),
				});
			case TransformErrorTypes.UnsupportedCSS:
				return t("element.html.errors.unsupportedCSS", {
					css: error.css.substring(0, CSS_MAX_LENGTH),
				});
			case TransformErrorTypes.UnsupportedCSSAttribute:
				return t("element.html.errors.unsupportedCSSAttribute", {
					prop: error.prop,
				});
			case TransformErrorTypes.UnsupportedCSSSelector:
				return t("element.html.errors.unsupportedCSSSelector", {
					selector: error.selector,
				});
			case TransformErrorTypes.UnsupportedCSSValue:
				return t("element.html.errors.unsupportedCSSValue", {
					prop: error.prop,
					value: error.value,
				});
			...
			case TransformErrorTypes.UnsupportedHTMLTagName:
				return t("element.html.errors.unsupportedHTMLTagName", {
...
export function InputHTML({ disabled, name, label }: InputHTMLProps) {
	...
	const getCombinedErrors = (): FormErrors => {
		...
		const errorsStr: Array<string | null> = errors.errors.map(
			transformError(t),
		);
