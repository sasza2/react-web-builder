...
export function OpenContainer({ id, testId }: OpenContainerProps) {
	...
	const onOpen = () => {
		if (containerId) {
			...
			const containerExtras = getContainerExtras(
				container,
				gridAPIRef.current.getPanZoom().getZoom(),
			);
			...
			const tree = createTreeElements(
				...
				container.cols,
				getBreakpointRowsByLastElement(
					elementsInBreakpoints[container.id],
					elementsExtras.current[container.id] || {},
				),
			);
	...
	return (
		<FormGroup
			data-id="openContainer"
			{...assignTestProp(testId, null, "openContainer")}
		>
			<FormHeader>{t("container.content")}</FormHeader>
			<Description>{t("container.description")}</Description>
			<LinkButton onClick={onOpen}>{t("container.button")}</LinkButton>
			<Hints list={hintsList} speed={0} />
		</FormGroup>
	);
}
