...
export function BreakpointHeight({ name, testId }: BreakpointHeightProps) {
	...
	return (
		<FormGroup
			data-id="breakpointHeight"
			{...assignTestProp(testId, null, "breakpointHeight")}
		>
			<FormHeader>{t("breakpoint.heightProp.height")}</FormHeader>
			<Toggle
				name={`${name}.enabled`}
			...
			{enabled && (
				<>
					...
					<Toggle
						...
						description={t("breakpoint.heightProp.responsive.description", {
			...
			{value?.overflow === "scroll" && (
				<Toggle
					...
					description={t(
						"breakpoint.heightProp.scrollbarVisibility.description",
					)}
				/>
			)}
		</FormGroup>
