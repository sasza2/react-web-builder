...
import type { Breakpoint, Padding as IPadding } from "types";
	...
	const renderField = useMemo(() => {
		switch (field) {
			case "backgroundColor":
				return (
					<FormGroup {...assignTestProp(testId)}>
						<FormHeader>{t("breakpoint.background")}</FormHeader>
						<FieldProvider name={field} setValue={setValue} value={value}>
							<ColorPicker
								allowGradient
								name={field}
			...
			case "cols":
				return (
					<FormGroup {...assignTestProp(testId)}>
						<FormHeader>{t("breakpoint.numberOfColumns")}</FormHeader>
						<FieldProvider name={field} setValue={setCols} value={value}>
							<Input name={field} />
						</FieldProvider>
					</FormGroup>
				);
