...
import { ColorsContainer, InputContainer } from "./ColorPicker.styled";
	...
	return (
		<FormControl name={name} label={label} errors={errors} testId="colorPicker">
			<InputContainer onClick={onSketchOpen} ref={inputContainerRef}>
				<Color color={mainColor()} active size="lg" />
			...
			{isSketchOpen && (
				<FieldProvider name="color" setValue={onSketchSetValue} value={value}>
					<ColorPickerModal
						allowGradient={allowGradient}
						closeOnlyOnClickOutsideSidebarModal={
							closeOnlyOnClickOutsideSidebarModal
