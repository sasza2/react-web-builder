...
export function Color({ active, color, size, onClick }: ColorProps) {
	const id = useId();
	...
	return (
		<ColorContainer
			...
			{...assignTestProp(
				"color",
				null,
				isGradient ? null : getColorForTooltip(color),
			)}
		>
			<ColorDiv $color={color} $size={colorSize} />
			{!isGradient && (
				<Tooltip id={tooltipId}>{getColorForTooltip(color)}</Tooltip>
			)}
		</ColorContainer>
	);
}
