...
export function BoxShadow({ name, testId }: BoxShadowProps) {
	...
	const setBoxShadowValue =
		<T extends keyof IBoxShadow>(field: T) =>
		(nextValue: IBoxShadow[T]) => {
	...
	return (
		<FormGroup {...assignTestProp(testId, null, "boxShadow")}>
			<FormHeader>{t("element.boxShadow.name")}</FormHeader>
			<FieldProvider name="enabled" setValue={toggleEnabled} value={enabled}>
				<Toggle name="enabled" />
			</FieldProvider>
			{enabled && (
				<>
					...
					>
						<Toggle name="inset" label={t("element.boxShadow.inset")} />
					</FieldProvider>
					<FieldProvider
