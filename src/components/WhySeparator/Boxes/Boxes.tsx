...
function BoxesIn({
	...
	const onTextNodeInit = useCallback(
		(node: HTMLDivElement) => {
			if (!node) return;
	...
	const box2 = (
		<Box2 ref={onTextNodeInit}>
			...
			tellus in libero sollicitudin eleifend.
		</Box2>
	);

	const box1 = <Box1 style={{ height }} />;
	...
	if (hasSeparator) {
		return <ContainerSeparator>{renderBoxes()}</ContainerSeparator>;
	}

	return <ContainerWithout>{renderBoxes()}</ContainerWithout>;
}
