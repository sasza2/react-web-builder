...
export function FormProperty({
	...
	if (prop.type === "fontOptions") {
		return (
			<FormGroup {...assignTestProp(testId, null, prop.type)}>
				<FormHeader>{prop.label}</FormHeader>
	...
	if (prop.type === "list") {
		return (
			<FormGroup {...assignTestProp(testId, null, prop.type)}>
				<FormHeader>{prop.label}</FormHeader>
				<ListOrder name={name} options={prop.options} />
			</FormGroup>
		);
	}
	if (prop.type === "padding") {
		return <Padding name={name} testId={testId} />;
	}
	if (prop.type === "border") {
		return <Border name={name} testId={testId} />;
	}
	...
	if (prop.type === "boxShadow") {
		return <BoxShadow name={name} testId={testId} />;
	}
	if (prop.type === "select") {
		return (
			<FormGroup {...assignTestProp(testId, null, prop.type)}>
				<FormHeader>{prop.label}</FormHeader>
				<Select name={name} size="lg" options={prop.options} />
			</FormGroup>
		);
	}
	...
	if (prop.type === "text") {
		return (
			<FormGroup {...assignTestProp(testId, null, prop.type)}>
				<FormHeader>{prop.label}</FormHeader>
				<Input leftNode={prop.leftNode} label={prop.description} name={name} />
			</FormGroup>
		);
	}
	if (prop.type === "url") {
		return (
			<FormGroup {...assignTestProp(testId, null, prop.type)}>
				<FormHeader>{prop.label}</FormHeader>
				<URLInput name={name} canOpenInNewTab={prop.canOpenInNewTab} />
			</FormGroup>
		);
	}
	if (prop.type === "color") {
	...
	if (prop.type === "about") {
		return (
			<FormGroup {...assignTestProp(testId, null, prop.type)}>
				<FormHeader>{prop.label}</FormHeader>
				<About button={prop.button} description={prop.description} />
			</FormGroup>
		);
	}
	if (prop.type === "img") {
