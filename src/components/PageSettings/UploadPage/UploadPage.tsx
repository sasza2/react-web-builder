...
import {
	...
	FormGroup,
	FormHeader,
} from "@/components/forms/FormControl.styled";
...
export function UploadPage() {
		...
		const onChange = (changeEvent: Event) => {
			...
			if (file) {
				...
				reader.onload = (e: ProgressEvent<FileReader>) => {
					...
					restartTemplate(page).then(resolve).catch(reject);
				};
	...
	return (
		<FormGroup>
			<FormHeader>{t("page.settings.upload.title")}</FormHeader>
			<Description>{t("page.settings.upload.description")}</Description>
			<LinkGhostButton onClick={onUploadClick}>
