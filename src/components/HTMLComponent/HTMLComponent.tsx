...
const RenderNodeChild = memo(({ child }: RenderNodeChildProps) => {
	switch (child.type) {
		...
		case "element": {
			if (
				!ALLOWED_TAGS_VALUES.includes(
					child.tagName as (typeof ALLOWED_TAGS_VALUES)[number],
				)
			)
				return null;

			const children = child.children.length
				? child.children.map((currentChild) => (
...
export const HTMLComponent = memo(({ element, html }: HTMLComponentProps) => {
	...
	return (
		<div className={className} style={{ display: "grid", overflow: "hidden" }}>
			{nodes.map((child) => (
				<RenderNodeChild key={child.key} child={child} />
			))}
		</div>
	);
});
