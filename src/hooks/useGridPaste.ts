...
export const useGridPaste = () => {
	...
	const paste = (x: number, y: number) => {
		...
		switch (obj.type) {
			case "element": {
				const { breakpoint: clipboardBreakpoint, element } =
				...
				dispatch(
					pasteElements({ currentBreakpoint: breakpoint, elementsTree }),
				);
			...
			case "tree": {
				...
				organizeElementsDelay({
					addGap:
						clipboardBreakpoint.cols === breakpoint.cols
							? 0
							: MARGIN_BOTTOM_ON_PASTED_ELEMENT,
					pastedElements,
				});
				break;
			}
