...
const areElementsEqual = (a: WebBuilderElement, b: WebBuilderElement) => a.x === b.x && a.y === b.y && a.w === b.w;
  ...
  const set: GridProps['setElements'] = (gridElements, { type }) => {
    ...
    const nextElements = gridElements.map((gridElement) => {
      ...
      if (!currentElements[element.id]) {
        ...
        if (!areElementsEqual(currentElement, element as unknown as WebBuilderElement)) {
          hasChanged = true;
        }
      }
