...
export const useInitI18n = () => {
  const { translations } = useProperties() as { translations: Record<string, string> };
  const [locale, setLocale] = useState(() => getDefaultLanguage(translations));

  const [instanceI18n, setInstanceI18n] = useState<typeof i18n>(() => initI18n(translations).instance);

  useEffect(() => {
    if (!translations || !translations.locale || translations.locale === locale) return;
